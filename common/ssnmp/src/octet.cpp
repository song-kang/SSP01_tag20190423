...
const char *OctetStr::get_printable_hex() const
{
	...
	//MEMO: 格式化成HEX，无空格 [2016-7-13 15:49 邵凯田]
	storageNeeded = (unsigned int)smival.value.string.len*2+1;
	OctetStr *ncthis = PP_CONST_CAST(OctetStr*, this);
	...
	for(int i=0;i<smival.value.string.len;i++)
	{
		sprintf(line_ptr, "%02X", *bytes);
		line_ptr += 2;
		bytes++;
	}
	output_buffer[storageNeeded-1] = '\0';
