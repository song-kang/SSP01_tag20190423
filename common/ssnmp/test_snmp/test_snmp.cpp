...
int main(int argc,char* argv[])
{
	...
	SSnmpClient client;
	client.SetVersion(SSNMP_VER3);
	SSnmpClient::GlobalInit();
	client.SetServerAddress("193.110.22.141");
	client.SetCommunity("public");
	for(int ss=0;ss<1;ss++)
	{
		if(!client.BeginSession())
		{
			printf("BeginSession error!");
			return 0;
		}
		stuSnmpSwitchInfo switchinfo;
		client.GetSwitchInfo(&switchinfo);
		SPtrList<stuSSnmpSwitchPortInfo>* pSwitchPortInfo;
		//client.GetSwitchPortInfo(pSwitchPortInfo);
		SPtrList<stuSSnmpSwitchPortMacInfo> SwitchPortMacInfo;
		client.GetSwitchPortMacInfo(SwitchPortMacInfo);
		//¶Ë¿ÚÊýÁ¿1.3.6.1.2.1.2.1.0
		SString oids = "1.3.6.1.2.1.2.2.1.1";
		if(argc>1)
			oids = argv[1];
		SString sValue;
		for(int i=0;i<1;i++)
		{
			...
			gets(buf);
			sValue = buf;
			client.Get(oids,sValue);
		}

		client.EndSession();

	}
	SSnmpClient::GlobalExit();
	getchar();
	return 0;
}
