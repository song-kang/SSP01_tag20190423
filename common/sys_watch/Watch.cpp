...
#define ICCHMI_RUNNING_LAMP_SWITCH 0 //一体化管控工具设备上的运行灯开关
#if ICCHMI_RUNNING_LAMP_SWITCH
#include "uklib.h"
#endif
//////////////////////////////////////////////////////////////////////
...
void CWatch::Watch()
{
	...
	for(m=0;m<cnt;m++)
	{
		...
		for(i=0;i<ps;i++)
		{
			...
			if(p < 0)
				continue;
			printf("sTemp=[%s]    sModule=[%s]\n",sTemp.data(),sModule.data());
			if(sTemp.length() == p+sModule.length() ||
				 sTemp.at(p+sModule.length()) == ' ')
			{
				pid  = SString::GetIdAttributeI(1,sModuleInfo,",");
			...
			if(sModule == "IccHmi")
			{
				unsigned long status = 1;//0表示关闭，1表示打开，2表示取反
				unsigned int lednum = 0;//0表示运行灯序号
				long lRst = UK_UsrLed(lednum,status);
				LOGWARN("指示灯序号%d,设置状态%ld，设置结果:%ld",lednum,status,lRst);
		...
		if(!bRun)
		{
			...
			if(sModule == "IccHmi")
			{
				unsigned long status = 0;//0表示关闭，1表示打开，2表示取反
				unsigned int lednum = 0;//0表示运行灯序号
				long lRst = UK_UsrLed(lednum,status);
				LOGWARN("指示灯序号%d,设置状态%ld，设置结果:%ld",lednum,status,lRst);
			} 
