...
typedef struct stuSMmsClient_UserInfo_2
{
	void *pMmsClient;
...
ST_CHAR varName [MAX_IDENT_LEN + 1];
  ...
  if (ret == SD_SUCCESS)
	...
	if (ret != SD_SUCCESS)
	{
		printf("named_var_write OptFlds error, dom=%s,var=%s\n",domName,varName);
		ret = SD_SUCCESS;
	}
    }
  ...
  if (ret == SD_SUCCESS)
    ...
    ret = named_var_write (netInfo, varName, DOM_SPEC, domName, rpt_typeids->bstr6/*bstr8*/, TrgOps, timeOut);
	if(ret != SD_SUCCESS)
	{
		printf("写报告(%s,%s)的触发选项TrgOps失败!\n",domName,rcbName);
		ret = SD_SUCCESS;
	}
    }
...
ST_RET retcode = SD_SUCCESS;
  ...
  if(event->net_info->user_info == NULL)
	  all_rcb_info = NULL;
  else
	all_rcb_info = ((struct stuSMmsClient_UserInfo_2*)event->net_info->user_info)->pALL_RCB_INFO;
  ...
  va_num=0;
  if (va_num >= va_total) goto CLEANUP;
  ...
  */
  ...
  if (va_num!=va_total)
    ...
    //del by skt u_iec_rpt_ind_data (info_va, OptFldsData, InclusionData, rcb_info, va_total);
	SMms_iec_rpt_ind_data (event->net_info->user_info, info_va, OptFldsData, InclusionData, rcb_info, va_total);
    }
