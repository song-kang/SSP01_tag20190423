...
#include "iec_demo.h"	/* definitions used from "iecgoose" sample	*/
...
/* message is received.							*/
/* CUSTOMIZED for "scl_srvr" sample application.			*/
/************************************************************************/
ST_VOID u_goose_ind (SN_UNITDATA *sn_udt)
  {
ST_RET retcode;
  printf ("\n Received IEC 61850 GOOSE Message.");
  /* NOTE: To do this, you must include "iec_demo.h" above and		*/
  /*       link in code from the "iecgoose" directory.			*/
  retcode = chk_for_goose_msg (sn_udt);
  if (retcode)
    printf ("\n GOOSE Message error detected in 'chk_for_goose_msg'.");
  }
