...
/*									*/
/* MODULE NAME : crc_util.c						*/
/* PRODUCT(S)  : General Use						*/
/*									*/
/* MODULE DESCRIPTION :							*/
/*	General purpose CRC compurartion functions.			*/
/*									*/
...
/* --------  ---  ------   -------------------------------------------  */
/* 11/17/09  JRB    02     Make crc32table external.			*/
...
/*			     sCrc32OnString				*/
/*----------------------------------------------------------------------*/
/* Compute 32-bit CRC for a string.					*/
/* The CRC may be computed for a series of strings. In this case the	*/
/* crc from a previous computation should be passed to the next		*/
/* computation.	The initial crc should be set to 0.			*/
/* Return:								*/
/*	ST_UINT32		the computed 32-bit CRC			*/
/************************************************************************/
...
ST_UCHAR *uc;
  ...
  uc = (ST_UCHAR *) str;
    ...
    /* byte loop */
  while (*uc)
    crc = (((crc >> 8) & 0x00FFFFFF) ^ crc32table[(crc ^ *(uc++)) & 0x000000FF]);
...
/*			     sCrc32OnBuffer				*/
/*----------------------------------------------------------------------*/
/* Compute 32-bit CRC for a buffer of characters.			*/
/* The CRC may be computed for a series of buffers. In this case the	*/
/* crc from a previous computation should be passed to the next		*/
/* computation.	The initial crc should be set to 0.			*/
/* Return:								*/
/*	ST_UINT32		the computed 32-bit CRC			*/
