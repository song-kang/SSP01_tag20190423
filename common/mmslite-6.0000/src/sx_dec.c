...
/* or block of binary CDATA						 */
    ...
    /* We have a element tag start, get the tag  first  */
      ...
      while (SX_RIP_NOT_DONE)
        {
        c = *xml;
        if (c == '>' || c == '/' || isspace ((int)(unsigned char)c)) 	/* Found the end of the tag *///MEMO: isspaceÀàÐÍ×ª»»£¬±ÜÃâÀàÐÍ´íÎó [2016-8-18 18:20 ÉÛ¿­Ìï]
          break;
	...
	while (isspace ((int)(unsigned char)*xml) && SX_RIP_NOT_DONE)//MEMO: isspaceÀàÐÍ×ª»»£¬±ÜÃâÀàÐÍ´íÎó [2016-8-18 18:20 ÉÛ¿­Ìï]
          xml++;
        ...
        /* This should be the start of an attribute name */ 
          ...
          while (SX_RIP_NOT_DONE)
            {
            c = *xml;
            if (c == '=' || isspace ((int)(unsigned char)c))	/* Found the end of the attribute name *///MEMO: isspaceÀàÐÍ×ª»»£¬±ÜÃâÀàÐÍ´íÎó [2016-8-18 18:20 ÉÛ¿­Ìï]
              break;
        ...
        /* skip to the attribute '=' */
		  ...
		  quotes_ch = *(xml);//MEMO: ¼ÇÂ¼attrÊôÐÔÖµ¶¨½ç·û£¬·ÀÖ¹Ë«ÒýºÅÓëµ¥ÒýºÅ»ìÓÃÊ±´íÎó [2016-8-18 18:36 ÉÛ¿­Ìï]
          xml++;
          ...
          while (SX_RIP_NOT_DONE)
            {
            c = *xml;
            //MEMO:  [2016-8-18 18:37 ÉÛ¿­Ìï]if (c == '"' || c == '\'')	/* Found the end of the attrib */
			if (c == quotes_ch)//MEMO: ·ÀÖ¹Ë«ÒýºÅÓëµ¥ÒýºÅ»ìÓÃÊ±´íÎó [2016-8-18 18:38 ÉÛ¿­Ìï]
              break;
          ...
          //MEMO:  [2016-8-18 18:37 ÉÛ¿­Ìï]if (*(xml) != '"' && *(xml) != '\'')		/* skip the closing " */
		  if(*(xml) != quotes_ch)//MEMO: ·ÀÖ¹Ë«ÒýºÅÓëµ¥ÒýºÅ»ìÓÃÊ±´íÎó [2016-8-18 18:38 ÉÛ¿­Ìï]
            {
            SXLOG_NERR0 ("SX decode error: could not find closing attribute value '\"'");
            sxDecCtrl->errCode = SD_FAILURE;
            return (SD_FAILURE);
	...
	bEmptyTag = SD_FALSE;
      ...
      while (SX_RIP_NOT_DONE)
        {
        c = *xml;
        if (c == '>' || isspace ((int)(unsigned char)c))	/* Found the end of the tag *///MEMO: isspaceÀàÐÍ×ª»»£¬±ÜÃâÀàÐÍ´íÎó [2016-8-18 18:20 ÉÛ¿­Ìï]
          break;
