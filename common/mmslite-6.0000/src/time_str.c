...
/*			   Always chk localtime, gmtime return.		*/
/*			   Del unused CalculateTimeZoneOffset (didn't	*/
/*			   seem to handle DST correctly anyway).	*/
/* 02/03/10  NAV     42    OpenVMS port					*/
...
ST_RET tstrTimeToString (time_t t, ST_CHAR *dest, size_t maxsize)
  {
  struct tm *tm_info;
  tm_info = localtime (&t);
  if (tm_info == NULL)
  ...
  strftime (dest, maxsize, TSTR_DEF_TIME_FORMAT, tm_info);
  return (SD_SUCCESS);
  }
...
ST_RET tstrTimeToStringGmt (time_t t, ST_CHAR *dest, size_t maxsize)
  {
  struct tm *tm_info;
  tm_info = gmtime (&t);
  if (tm_info == NULL)
...
ST_RET tstrTmToString (struct tm *t, ST_CHAR *dest, size_t maxsize)
  {
  ST_CHAR *timeFormatStr = "%m-%d-%Y %H:%M:%S";
  strftime (dest, maxsize, timeFormatStr, t);
  return (SD_SUCCESS);
  }
...
/************************************************************************/
/*			BtimeString					*/
/* Build string (same for Btime4 or Btime6).				*/
/************************************************************************/
static ST_RET BtimeString (char *dest,	/* ptr to destination buffer	*/
	size_t maxsize,	/* destination buffer size			*/
	time_t timer,	/* time in seconds				*/
	ST_LONG msec)	/* milliseconds in the current second		*/
  {
struct tm *tm_info;
char strn_msec [22];	/* "msec" converted to strings			*/
			/* only need 4 char for ".123", but if "msec"	*/
			/* is bad, sprintf could generate up to 21 char	*/
			/* plus NULL on 64-bit system.			*/
  ...
  /* Leave room to append 4 char "strn_msec".	*/
  strftime (dest, maxsize-4, BTIME_DEF_TIME_FORMAT, tm_info);

  /* Convert msec to 4 char string and append it.		*/
  /* If strlen != 4 (invalid data), just leave off msec.	*/
  sprintf (strn_msec, ".%03lu", (ST_ULONG) msec);
  if (strlen (strn_msec) == 4)	/* should never fail if data is good	*/
    strcat (dest, strn_msec);	/* append "strn_msec"	*/
  return (SD_SUCCESS);
  }
...
_TS_DATETIME dt;
  ...
  /* Convert to string in "dest" buffer.	*/
  return (BtimeString (dest, maxsize, tmRslt, balMSec));
  } /* Btime6ValsToString */
...
time_t theTime;
  ...
  if (curTime==NULL)	/* this should NEVER happen	*/
    return (SD_FAILURE);
...
struct tm *pTm;
  ...
  /* Leave room to append 10 char "pFract" & 16 char "theQual".	*/
  strftime (dest, maxsize-26, UTC_DEF_TIME_FORMAT, pTm);

  /* get the fraction portion */

  if (fraction >= 0x01000000)
    {
    SLOGALWAYS1 ("UtcToString - fraction %lu out of range", (ST_ULONG) fraction);
    return SD_FAILURE;
    }

  /* Because of "fraction" check above, "dFraction" is always < 1.	*/
  dFraction = ((ST_DOUBLE) fraction / (ST_DOUBLE) 0x01000000);
  sprintf (theFraction, " %#0.09f", dFraction);
  pFract = strchr (theFraction, '.');

  if (!pFract || strlen (pFract) != 10)
    {
  ...
  strcat (dest, pFract);	/* append fraction	*/
  strcat (dest, theQual);	/* append quality	*/
...
#define MAX_DATE_TIME_STR_LEN	32	/* longest output format:		*/
  ...
  if (sxDateTime->useTZ && sxDateTime->tz == 0)
    tm_info = gmtime (&sxDateTime->dateTime);
  else
    tm_info = localtime (&sxDateTime->dateTime);
  if (tm_info == NULL)
    return (SD_FAILURE);

  /* Leave room to append 7 char "theFraction" & 6 char "theTZ".	*/
  strftime (dest, destLen-13, "%Y-%m-%dT%H:%M:%S", tm_info);
  ...
  if (sxDateTime->useMicroseconds)
    ...
    if (strlen (theFraction) > 7)	/* should never happen if data is good*/
      strcpy (theFraction, ".??????");
  ...
  if (sxDateTime->useTZ)
	...
	tzValue = -tzValue;
      ...
      if (strlen (theTZ) > 6)	/* should never happen if data is good*/
        strcpy (theTZ, "+??:??");
      strcat (dest, theTZ);
      }
    }
