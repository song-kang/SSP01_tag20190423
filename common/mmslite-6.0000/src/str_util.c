...
/* 03/02/12  JRB	   strncat_maxstrlen: ret err if result truncated.*/
/* 11/23/11  KCR           Added casting for proper 64bit compile       */
/* 03/31/11  EJV           asciiToSint32: chg 0x80000000 to (-214...-1),*/
/*                         on Sun 64-bit (LP64) was not casted properly.*/
...
/* RETURNS: SD_SUCCESS or SD_FAILURE if result is truncated.		*/
/************************************************************************/
ST_RET strncat_maxstrlen (char *dest, char *src, size_t maxstrlen)
  ...
  src_len = strlen (src);
  if (dest_len + src_len <= maxstrlen)
    {
    strcat (dest, src);	/* always safe	*/
    retcode = SD_SUCCESS;	/* no truncation	*/
    }
  else
    {
    /* If dest is smaller than max, copy as much as possible.	*/
    if (dest_len < maxstrlen)
      {
      strncat (dest, src, maxstrlen-dest_len);	/* always NULL-terminates dest*/
      }
    retcode = SD_FAILURE;	/* result is truncated	*/
    }
  return (retcode);
  }
...
ST_CHAR *retPtr;
  ...
  *uLong = 0; 
  ...
  //MEMO: add by skt, ÈÝ´í£¬¿Õ×Ö·û´®×ª0 [2016-8-18 18:15 ÉÛ¿­Ìï]
  if(astr[0] == '\0')
  {
	  *uLong = 0;
	  return SD_SUCCESS;
...
ST_LONG sLong = 0;
  ...
  *value = 0;
  ...
  if (sLong > INT32_MAX || sLong < (-2147483647-1))
...
ST_INT     copyLen;
  ...
  if ((fd = fopen (fileStr, "r")) != NULL)
    ...
    while (fgets(lineBuf, sizeof(lineBuf)-1, fd) != NULL)
      ...
      if (keywordPtr)
        {
	copyLen = min (valBufLen-1, (int) (strlen(lineBuf)-strlen(keywordStr)));
