    ...
    if (map_entry)
      {
		  ...
		  else if(map_entry->user_map != NULL)
		  {
			  int reason = 0;
			  if (mvluRdVaCtrl->indCtrl->scan_va_done_fun && mvluRdVaCtrl->indCtrl->usr_resp_fun)
				  reason = 0;		// mvl61850_rpt_change_scan_done
			  else if (mvluRdVaCtrl->indCtrl->scan_va_done_fun == NULL && mvluRdVaCtrl->indCtrl->usr_resp_fun)
				  reason = 1;		// mvl61850_log_change_scan_done
			  else
				  reason = 2;
			  if(!g_MmsServer_ptr->OnReadLeafValue(map_head_sorted->ldevice,map_entry->leaf,map_entry->user_map,mvluRdVaCtrl,reason))
			  {
			  }
		  }
      }
