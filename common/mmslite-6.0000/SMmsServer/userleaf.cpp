...
extern "C" {
#endif
	extern SMmsServer *g_MmsServer_ptr;
#ifdef DEBUG_SISCO
SD_CONST static ST_CHAR *SD_CONST thisFileName = __FILE__;
#endif

bool g_bGlobalCtrl_Select_En=true;
bool g_bGlobalCtrl_Oper_En=true;
bool g_bGlobalCtrl_Cancel_En=true;
...
ST_CHAR branch_name[MAX_IDENT_LEN+1];	/* use to track struct nesting	*/
  ...
  for (j = 0, rt_type = rt_ctrl->rt_first;
       ...
       j++, rt_type++)
    {
    comp_name = (char*)ms_comp_name_find(rt_type);
    ...
    if (ms_is_rt_prim (rt_type))
      {
      if (/*del by skt at 20170515 strncmp (branch_name, "$ST", 3) == 0 ||*/
...
ST_CHAR *comp_name;
  ...
  if (map_head_sorted == NULL)
     ...
     */
		...
		if(map_entry != NULL && map_entry->user_map != NULL)
		{
			g_MmsServer_ptr->OnWriteLeafValue(map_head_sorted->ldevice,map_entry->leaf,map_entry->user_map,mvluWrVaCtrl);
		}
    }

  comp_name = (char*)ms_comp_name_find (mvluWrVaCtrl->rt);
...
ST_VOID u_ctl_oper_ctlval_wr_ind (MVLU_WR_VA_CTRL *mvluWrVaCtrl)
  {
ST_RET retcode;
//MEMO: 控制Oper入口1 [2017-4-7 15:17 邵凯田]
  if (!mvluWrVaCtrl->wrVaCtrl->is_61850_control || !g_bGlobalCtrl_Oper_En)
    {
    /* ERROR: Not writing whole "Oper" structure.	*/
    mvluWrVaCtrl->wrVaCtrl->failure = ARE_OBJ_ACCESS_DENIED;	/* send in resp*/

	char path[2048] = {'\0'};
	strcat(path, mvluWrVaCtrl->wrVaCtrl->va_scope.dom->name);
	strcat(path+strlen(path), "/");
	strcat(path+strlen(path), mvluWrVaCtrl->wrVaCtrl->va->name);
	g_MmsServer_ptr->OnReturnOper(path, mvluWrVaCtrl->primData, false);
  ...
  strcat(path+strlen(path), "/");
  strcat(path+strlen(path), mvluWrVaCtrl->wrVaCtrl->va->name);
  if (g_MmsServer_ptr->OnReturnOper(path, mvluWrVaCtrl->primData, retcode==SD_SUCCESS?true:false))
	  retcode = SD_SUCCESS;
  else
	  retcode = SD_FAILURE;
...
ST_VOID u_ctl_oper_other_wr_ind (MVLU_WR_VA_CTRL *mvluWrVaCtrl)
  {
	  //MEMO: 控制Oper入口2 [2017-4-7 15:17 邵凯田]
ST_CHAR *comp_name;
  if (!mvluWrVaCtrl->wrVaCtrl->is_61850_control || !g_bGlobalCtrl_Oper_En)
    {
    /* ERROR: Not writing whole "Oper" structure.	*/
...
ST_VOID u_cancel_comp_wr_ind (MVLU_WR_VA_CTRL *mvluWrVaCtrl)
  {
	  //MEMO: 控制Cancel入口 [2017-4-7 15:16 邵凯田]
ST_CHAR *comp_name;
  if (!mvluWrVaCtrl->wrVaCtrl->is_61850_control || !g_bGlobalCtrl_Cancel_En)
    {
	...
	strcat(path+strlen(path), "/");
	strcat(path+strlen(path), mvluWrVaCtrl->wrVaCtrl->va->name);
	g_MmsServer_ptr->OnReturnCancel(path, mvluWrVaCtrl->primData, false);
  ...
  comp_name = (char*)ms_comp_name_find (mvluWrVaCtrl->rt);
  if (strcmp (comp_name, "ctlVal") == 0)
  {
	  char path[2048] = {'\0'};
	  strcat(path, mvluWrVaCtrl->wrVaCtrl->va_scope.dom->name);
	  strcat(path+strlen(path), "/");
	  strcat(path+strlen(path), mvluWrVaCtrl->wrVaCtrl->va->name);
	  g_MmsServer_ptr->OnReturnCancel(path, mvluWrVaCtrl->primData, true);
  }
  ...
  if (mvluWrVaCtrl->wrVaCtrl->sboCtrl)
    ...
    mvlu_wr_prim_done (mvluWrVaCtrl, SD_SUCCESS /*SD_FAILURE*/);
...
MVL_SBO_CTRL *sboCtrl;
//MEMO: 控制Select入口 [2017-4-7 15:15 邵凯田]
  if (!mvluWrVaCtrl->wrVaCtrl->is_61850_control || !g_bGlobalCtrl_Select_En)
    {
	...
	strcat(path+strlen(path), "/");
	strcat(path+strlen(path), mvluWrVaCtrl->wrVaCtrl->va->name);
	g_MmsServer_ptr->OnReturnSelect(path, mvluWrVaCtrl->primData, false);
  ...
  if (strcmp (comp_name, "Check") == 0)
    ...
    if (sboCtrl)
      ...
      retcode = SD_SUCCESS;//SD_FAILURE;	/* 'negative' resp*/
	...
	strcat(path+strlen(path), "/");
	strcat(path+strlen(path), mvluWrVaCtrl->wrVaCtrl->va->name);
	if (g_MmsServer_ptr->OnReturnSelect(path, mvluWrVaCtrl->primData, true))
		retcode = SD_SUCCESS;
	else
		retcode = SD_FAILURE;
    }
