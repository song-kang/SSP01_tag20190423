...
ST_UINT ld_idx;			/* array index	*/
  ...
  while (fgets (in_buf, sizeof(in_buf) - 1, in_fp) != NULL)
    ...
    if (!map_ld)	/* struct NOT found	*/
      ...
      map_ld = (DATA_MAP_LD*)chk_calloc (1, sizeof (DATA_MAP_LD));
  ...
  for (ld_idx = 0; ld_idx < ld_count; ld_idx++)
    ...
    map_ld->map_arr = (DATA_MAP**)chk_calloc (map_ld->map_count, sizeof (DATA_MAP *));
    for (map_idx = 0; map_idx < map_ld->map_count; map_idx++)
      {
      map_ld->map_arr[map_idx] = (DATA_MAP*)chk_calloc (1, sizeof (DATA_MAP));
      }
    }
...
ST_RET retcode;  
  ...
  if (map_entry == NULL)
  { 
	  //不通过datamap.cfg文件直接映射 [2012-6-13 邵凯田]
	  map_entry = map_head_sorted->map_arr[track_info->primdata_num] = (DATA_MAP *)chk_malloc (sizeof (DATA_MAP));
	  memset(map_entry,0,sizeof(DATA_MAP));
	  //strcpy (map_entry->ldevice, dom_name);
...
#if 0//MEMO: 不使用自带的死区机制 [2016-11-11 14:53 邵凯田]
...
#endif//MEMO: 不使用死区机制 [2016-11-11 14:53 邵凯田]

//MEMO: del by skt 已经开辟空间并赋值 [2016-8-31 13:55 邵凯田]
//   if (map_entry != NULL)
//     {				/* this leaf is mapped	*/
...
//      */
//     map_head_sorted->map_arr [track_info->primdata_num] = (DATA_MAP*)chk_malloc (sizeof (DATA_MAP));
...
RUNTIME_TYPE *rt_type_last;	/* to make sure don't go past end of RUNTIME_TYPE arr*/
  ...
  map_head_sorted = (DATA_MAP_HEAD*)chk_calloc (1, sizeof (DATA_MAP_HEAD) + track_info.primdata_num * sizeof (DATA_MAP *));
...
DATA_MAP_LD *map_ld;		/* mappings from input file for one LD	*/
  ...
  for (didx = 0; didx < mvl_vmd.num_dom; didx++)
	...
	map_ld = map_ld_array[didx] = (DATA_MAP_LD*)chk_calloc (1, sizeof (DATA_MAP_LD));
	strcpy(map_ld->ldevice,dom->name);
	int map_count = 0;
	for (vidx = 0; vidx < dom->num_var_assoc; vidx++)
	{
		var = dom->var_assoc_tbl[vidx];
		//DATA_MAP_HEAD* map_head = (DATA_MAP_HEAD *) var->user_info;
		//map_count += map_head->map_count;
		map_count += var->arrCtrl.num_elmnts;
	}
	if(map_count == 0)
		map_count = 10;
	
	map_ld->map_arr = (DATA_MAP**)chk_calloc (map_count, sizeof (DATA_MAP*));
//     for (ld_idx = 0; ld_idx < ld_count; ld_idx++)
//       {
...
//       }
	...
	for (vidx = 0; vidx < dom->num_var_assoc; vidx++)
	{
		...
		if(map_head == NULL)
		{
			for(int j=0;j<(int)map_head->map_count;j++)
				map_ld->map_arr[map_count++] = map_head->map_arr[j];
		}
		else
		{
			for(int j=0;j<(int)map_head->map_count;j++)
...
//  datamap_cleanup (map_ld_array, ld_count);
	  ...
	  ST_CHAR *out_filename)		/* output cfg file name	*/
  {
	  ...
	  ST_UINT ld_idx;			/* current array index		*/
	  DATA_MAP_LD *map_ld;		/* mappings from input file for one LD	*/
	  DATA_MAP_LD map_ld_new;
	  memset(&map_ld_new,0,sizeof(map_ld_new));
	  /* Read "Mapping Cfg" file once to count entries & set up LD array.	*/
	  map_ld_array = datamap_cfg_alloc (in_filename, &ld_count);
	  if (map_ld_array)	/* may be NULL if file is empty	*/
	  {
