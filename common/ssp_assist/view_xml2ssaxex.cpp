...
void CViewXml2SSaxEx::PrepareNodeName(SBaseConfig *p)
{
	...
	int j;
	SString str;
	if(p->GetNodeName().find(":") >= 0)
		p->SetNodeName(p->GetNodeName().mid(p->GetNodeName().find(":")+1));
	if(p->GetNodeName().find("-") >= 0)
	{
		str = p->GetNodeName();
		str = str.replace("-","_");
	...
	{
		str = p->GetAttributePtr()->at(j)->m_sName;
		if(str.find(":") >= 0)
			p->GetAttributePtr()->at(j)->m_sName = str.mid(str.find(":")+1);
	}
	unsigned long pos;
	SBaseConfig *pSub = p->GetChildPtr()->FetchFirst(pos);
	while(pSub)
	{
		PrepareNodeName(pSub);
		pSub = p->GetChildPtr()->FetchNext(pos);
	}
}
...
// 	}
	...
	str += "{\r\n";
	str += "\tS_UNUSED(pAttribute);S_UNUSED(iAttributeCount);S_UNUSED(sNodeValue);\r\n";
