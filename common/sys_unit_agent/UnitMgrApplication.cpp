...
bool CUnitMgrApplication::Start()
{
	...
	if(!bOk)
	{
		LOGERROR("单元配置异常，单元IP(%s)不存在于本机的IP地址列表(%s)!",m_UnitConfig.m_sThisIp.data(),local_ips.data());
		return false;
	}
	...
	//m_Agent.SetExePath(GetBinPath());
	//m_Agent.SetUnitChkSelfSvr(&m_UnitChkSelfSvr);
	m_MCastFounder.SetUnitConfig(&m_UnitConfig);
	m_UnitChkSelfSvr.SetUnitConfig(&m_UnitConfig);
	if(!m_UnitChkSelfSvr.Start())
	{
		LOGFAULT("单元自检服务加载失败!");
		return false;
	}
...
// 	SString ret = GetNowModInfo();
// 	LOGDEBUG("GetNowModInfo()=%s",ret.data());
