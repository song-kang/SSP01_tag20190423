...
// 	}
	...
	SString m_sRunLog;//运行日志，多行使用换行分隔,
	SString m_sRunError;//运行错误描述，
};
...
public:
	SScriptExtPkg_System()
	{
		...
		m_ExtFunctionPtrs.append(new SScriptParser::stuExtFunctionParam("sleep",ExtFun_sleep));
	...
	static bool ExtFun_sleep(void *pCbParam,SScriptParser::stuValue &ReturnVal,SPtrList<SScriptParser::stuValue> *pParamValues)
	{
		if(pParamValues == NULL || pParamValues->count() == 0)
			return false;
		for(int i=0;i<pParamValues->at(0)->GetValue().toInt();i++)
